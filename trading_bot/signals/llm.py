...
- 기술적 신호(RSI 극단, 거래량 급증, 추세 전환)가 주된 판단 근거.
  펀더멘털 데이터(PER/PBR/ROE 등)가 제공되면 기술적 신호와 함께 종합 판단하되,
  기술적 신호를 우선한다. 뉴스·소문 등 외부 정보는 절대 추측하지 마라.
...
class ClaudeSignalClient:
    ...
    def _build_user_message(
        ...
        # 펀더멘털 블록 (제공된 경우에만)
        funda_block = ""
        funda = features.get("fundamentals")
        if funda:
            funda_lines = ["[펀더멘털 (최근 연간)]"]
            if funda.get("per") is not None:
                funda_lines.append(f"- PER: {funda['per']:.1f}배")
            if funda.get("pbr") is not None:
                funda_lines.append(f"- PBR: {funda['pbr']:.2f}배")
            if funda.get("roe") is not None:
                funda_lines.append(f"- ROE: {funda['roe']:.1f}%")
            if funda.get("debt_ratio") is not None:
                funda_lines.append(f"- 부채비율: {funda['debt_ratio']:.0f}%")
            if funda.get("eps_growth") is not None:
                funda_lines.append(f"- EPS 성장률: {funda['eps_growth']:+.1f}%")
            if funda.get("dividend_yield") is not None:
                funda_lines.append(f"- 배당수익률: {funda['dividend_yield']:.2f}%")
            if len(funda_lines) > 1:
                funda_block = "\n".join(funda_lines) + "\n\n"
